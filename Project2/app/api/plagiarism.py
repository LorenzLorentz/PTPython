...
async def launch_plagiarism_task(payload:PlagiarismTaskLaunchPayload, db_session:Session=Depends(get_db)):
    """发起查重检测"""
...
async def  get_plagiarism_result(task_id:int, db_user:UserModel=Depends(require_login), db_session:Session=Depends(get_db)):
    """查询查重结果"""
...
async def get_pagiarism_report(task_id:int, db_user:UserModel=Depends(require_login), db_session:Session=Depends(get_db)):
    """下载查重报告"""
    db_task = db.db_task.get_task(db=db_session, task_id=task_id)
    if db_task is None:
        raise APIException(status_code=404, msg="查重任务不存在")
