...
class PlagiarismTask(BaseModel):
    ...
    sim_abstract_list:Optional[dict] = Field(None, description="可视化摘要")
...
class PlagiarismTaskReportResponse(BaseModel):
    result:bool = Field(False, validation_alias="id", description="查重结果")
    sim_submission_id_list:List[int] = Field([], description="对比对象")
    sim_list:Optional[List[float]] = Field([], description="相似度评分")
    sim_abstract:Optional[dict] = Field(None, description="可视化摘要")
    model_config = ConfigDict(from_attributes=True)

"""Payload"""
class PlagiarismTaskLaunchPayload(BaseModel):
    problem_id:str = Field(..., description="题目编号")
    submission_id:int = Field(..., description="提交编号")
